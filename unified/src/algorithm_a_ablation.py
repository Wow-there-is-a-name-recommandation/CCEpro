...
class AlgorithmAPipeline:
    ...
    def _sample(
        ...
        if cfg.enable_reverse_guidance:
        ...
        for step_idx, t in enumerate(timesteps):
            ...
            if not has_forbidden:
                ...
                if cfg.debug_steps and step_idx in [0, n_total // 2, n_total - 1]:
                    print(
                        f"step={step_idx}, reverse_guidance={'off' if not cfg.enable_reverse_guidance else 'inactive'}, "
    ...
    def generate(self, prompt: str, seed: int = 0) -> torch.Tensor:
        ...
        if self.cfg.enable_filter:
            prompt_seq, scores = self._filter_prompt(p_raw)
        else:
            prompt_seq = p_raw
            p0_mean = p_raw.mean(dim=1).squeeze(0)
            scores = {}
            for c in self.concepts:
                mem = self.memory[c]
                scores[c] = concept_score(p0_mean.unsqueeze(0), mem.U).item()

        img = self._sample(prompt_seq, scores, seed=seed)
        return img
...
if __name__ == "__main__":
    ...
    pipe = AlgorithmAPipeline(device=str(dev), config=cfg)
