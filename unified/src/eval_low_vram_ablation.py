...
DEFAULT_ERASED_TEMPLATES = 20
DEFAULT_ERASED_SEEDS = 3
...
DEFAULT_RELATED_TEMPLATES = 10
DEFAULT_RELATED_SEEDS = 5
DEFAULT_N_COCO = 50
...
def main() -> None:
    ...
    parser.add_argument("--method", type=str, required=True,
                        choices=["sd_v1.4", "algorithm_a", "algorithm_a_ablation", "cosa_gba", "unified"])
    ...
    parser.add_argument("--fid_ref_dir", type=str, default="unified/eval_ablation/reference_sd")

    # Algorithm A ablation toggles
    parser.add_argument("--enable_filter", type=int, default=1,
                        help="For algorithm_a only: 1=use prompt filter, 0=disable it")
    parser.add_argument("--enable_reverse_guidance", type=int, default=1,
                        help="For algorithm_a only: 1=use reverse guidance, 0=disable it")

    # Generation subset overrides (use these to reduce runtime from the command line)
    parser.add_argument("--erased_templates", type=int, default=DEFAULT_ERASED_TEMPLATES)
    parser.add_argument("--erased_seeds", type=int, default=DEFAULT_ERASED_SEEDS)
    parser.add_argument("--related_templates", type=int, default=DEFAULT_RELATED_TEMPLATES)
    parser.add_argument("--related_seeds", type=int, default=DEFAULT_RELATED_SEEDS)
    args = parser.parse_args()

    cfg_erased = GenConfig(
        n_templates=args.erased_templates,
        n_seeds=args.erased_seeds,
        device=args.device,
        flush_every=args.flush_every,
    )
    cfg_related = GenConfig(
        n_templates=args.related_templates,
        n_seeds=args.related_seeds,
        device=args.device,
        flush_every=args.flush_every,
    )
    cfg_coco = GenConfig(
    ...
    if args.mode in ("generate", "all"):
        extra_args = {
            "edited_pipe": args.edited_pipe,
            "spm_dir": args.spm_dir,
            "enable_filter": bool(args.enable_filter),
            "enable_reverse_guidance": bool(args.enable_reverse_guidance),
        }
        gen_fn = make_generator(args.method, args.device, extra_args=extra_args)
    ...
    if args.mode in ("fid", "all"):
        ...
        if args.fid_ref_dir:
            ref_dir = Path(args.fid_ref_dir) / "sd_v1.4" / "coco"
        else:
